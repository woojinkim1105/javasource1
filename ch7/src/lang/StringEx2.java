...
public class StringEx2 {
	...
	public static void main(String[] args) {
		...
		// charAt(int index) : char => charAt 호출시 int 매개변수를 넣어서 호출하고 return값은 char 받는다
		System.out.println(str1.charAt(0));
		System.out.println(str1.charAt(str1.length()-1));
		
		String str3 = "자바 프로프그래밍";
		...
		System.out.println("indexOf(int ch, int fromIndex) : "+str2.indexOf('e',0)); //0번 위치에서 시작해서 찾아 : 찾아서 1
		System.out.println("indexOf(int ch, int fromIndex) : "+str2.indexOf('e',2)); //2번 위치에서 시작해서 찾아 : 못찾아서 -1
		...
		String result = ssn.substring(3); //3번자리부터 출력
		System.out.println(result);
		//subString(int beginIndex, int endIndex)  이상, 미만
		...
		//startsWith(String prefix, int toOffset) : prefix로 시작하느냐??
		...
		//contains(CarSequence s) : s 문자열이 포함되었는지 검사
		System.out.println("contains(CarSequence s) "+str2.contains("Hello"));
		...
		System.out.println(str5); //String은 변경불가
		...
		System.out.println(str6.equals("Hello World")); //공백도 엄연히 글자
		System.out.println(str6.trim());
				
		//static 메소드 : 클래스이름.메소드명, 클래스이름.상수
		...
		// toLowerCase() / toUpperCase() : 모든 문자열을 소문자/대문자 로 변경
		...
		System.out.println("compareTo()"+"aaa".compareTo("영")); //0
